...
class Seed2needTestFramework():
    ...
    def _initialize_chain(self, toPosPhase=False):
        ...
        if not toPosPhase and not is_powcache_valid:
            ...
            # - Nodes 2 and 3 mint one zerocoin for each denom (tot 6666 FARM) on block 301/302
            ...
            for peer in range(4):
                for j in range(20):
                    ...
                    if nBlocks == 301 or nBlocks == 302:
                        # mints 7 zerocoins, one for each denom (tot 6666 FARM), fee = 0.01 * 8
                        # consumes 27 utxos (tot 6750 FARM), change = 6750 - 6666 - fee
    ...
    def test_PoS_chain_balances(self):
        ...
        # - Nodes 2 and 3 have 6666 FARM worth of zerocoins
    ...
    def get_prevouts(self, node_id, utxo_list, zpos=False, nHeight=-1):
        ...
        :return: prevouts:         ({bytes --> (int, bytes, int)} dictionary)
                                   maps CStake "uniqueness" (i.e. serialized COutPoint -or hash stake, for zfarm-)
                                   to (amount, prevScript, timeBlockFrom).
                                   For zfarm prevScript is replaced with serialHash hex string.
        """
    ...
    def stake_block(self, node_id,
        ...
        for tx in vtx:
            if not fDoubleSpend:
                # assume txes don't double spend zFARM inputs when fDoubleSpend is false. It needs to

...
class ZerocoinSpendTest(Seed2needTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zfarm_bal, farm_bal):
            zfarm_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zfarm_bal)
            farm_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], farm_bal)
            return zfarm_bal, farm_bal
        ...
        zfarm_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zfarm_balance, 6666)
        ...
        zfarm_balance, balance = check_balances(denom_0, zfarm_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfarm_balance, balance = check_balances(denom_2, zfarm_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zfarm_balance, balance = check_balances(denom_3, zfarm_balance, balance)
        ...
        assert_raises_rpc_error(-26, "bad-txns-invalid-zfarm",
        ...
        _, _ = check_balances(denom_1, zfarm_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")

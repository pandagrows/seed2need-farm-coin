...
CZFarmStake::CZFarmStake(const libzerocoin::CoinSpend& spend)
{
...
int CZFarmStake::GetChecksumHeightFromMint()
{
...
int CZFarmStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZFarmStake::GetChecksum()
{
    return nChecksum;
}

// The zFARM block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZFarmStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZFarmStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zFARM staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZFarmStake::GetUniqueness()
{
    //The unique identifier for a zFARM is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZFarmStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZFarmStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zFARM that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZFARMOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zFARM output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zFARM", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZFARMOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zFARM output", __func__);
...
bool CZFarmStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZFarmStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzFARMTracker* zfarmTracker = pwallet->zfarmTracker.get();
    CMintMeta meta;
    if (!zfarmTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zfarmTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!FARM Stake
bool CFarmStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CFarmStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CFarmStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CFarmStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CFarmStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CFarmStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CFarmStake::GetUniqueness()
{
    //The unique identifier for a FARM stake is the outpoint
...
CBlockIndex* CFarmStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;

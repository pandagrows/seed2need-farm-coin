...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    UniValue zfarmObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zfarmObj.push_back(Pair(std::to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zfarmObj.push_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.push_back(Pair("zFARMsupply", zfarmObj));
    ...
    if (block.IsProofOfStake()) {
        ...
        stakeData.push_back(Pair("stakeModifierHeight", ((stake->IsZFARM()) ? "Not available" : std::to_string(
...
UniValue getchecksumblock(const UniValue& params, bool fHelp)
{
 ...
 /*           "  \"zFARMsupply\" :\n"
            ...
            "     \"10\" : n,           (numeric) supply of 10 zFARM denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zFARM denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zFARM denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zFARM denomination\n"
...
UniValue getaccumulatorwitness(const UniValue& params, bool fHelp)
{
    ...
    if (!GenerateAccumulatorWitness(pubCoin, accumulator, witness, nMintsAdded, strFailReason)) {
        receipt.SetStatus(_(strFailReason.c_str()), ZFARM_FAILED_ACCUMULATOR_INITIALIZATION);

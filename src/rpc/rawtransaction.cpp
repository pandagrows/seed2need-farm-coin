...
UniValue createrawzerocoinstake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nCreates raw zFARM coinstakes (without MN output). Only for regtest\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    // create the zerocoinmint output (one spent denom + three 1-zFARM denom)
    ...
    if (!pwalletMain->CreateZFARMOutPut(staked_denom, vOutMint[1], dMint))
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zfarm output");

    for (int i=2; i<5; i++) {
        if (!pwalletMain->CreateZFARMOutPut(libzerocoin::ZQ_ONE, vOutMint[i], dMint))
            throw JSONRPCError(RPC_WALLET_ERROR, "failed to create new zfarm output");
    }
    ...
    CTxIn newTxIn;
    // Use v2 spends for input (with v3 zfarm staking is disabled)
...
UniValue createrawzerocoinspend(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            ...
            "\nCreates raw zFARM public spend.\n" +
            HelpRequiringPassphrase() + "\n"

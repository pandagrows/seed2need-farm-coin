                       ...
                       int &nPaymentCount, int &nBlockStart, CBitcoinAddress &address, CAmount &nAmount)
{
    ...
    if (nAmount < 10 * COIN)
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid amount - Payment of %d is less than minimum 10 FARM allowed", FormatMoney(nAmount)));
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    ...
    CWalletTx wtx;
    if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 50 FARM collateral for proposal

            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 100,000 FARM.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 100,000 FARM.");
                    } else {
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        CTxIn in;
        if(!ZFARMModule::createInput(in, mint, hashTxOut, spendVersion)) {
...
void CWallet::setZWallet(CzFARMWallet* zwallet)
{
    zwalletMain = zwallet;
    zfarmTracker = std::unique_ptr<CzFARMTracker>(new CzFARMTracker(strWalletFile));
}

CzFARMWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZFarmAutoBackups(bool fEnabled)
{

...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zfarmspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 FARM at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zFARM send
enum ZerocoinSpendStatus {
    ZFARM_SPEND_OKAY = 0,                            // No error
    ZFARM_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZFARM_WALLET_LOCKED = 2,                         // Wallet was locked
    ZFARM_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZFARM_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZFARM_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZFARM_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZFARM_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZFARM_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZFARM_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZFARM_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZFARM_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZFARM_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZFARM_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZFARM_SPENT_USED_ZFARM = 14,                      // Coin has already been spend
    ZFARM_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZFARM_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZFarmBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZFARMOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzFARMTracker> zfarmTracker;
    ...
    void setZWallet(CzFARMWallet* zwallet)
    {
        zwalletMain = zwallet;
        zfarmTracker = std::unique_ptr<CzFARMTracker>(new CzFARMTracker(strWalletFile));
    }

    CzFARMWallet* getZWallet() { return zwalletMain; }
    ...
    void setZFarmAutoBackups(bool fEnabled)
    {
    ...
    /** zFARM reset */
    boost::signals2::signal<void()> NotifyzFARMReset;

...
public:
    ...
    bool WriteZFARMSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZFARMSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZFARMSeed_deprecated(uint256& seed);
    bool EraseZFARMSeed();
    bool EraseZFARMSeed_deprecated();

    bool WriteZFARMCount(const uint32_t& nCount);
    bool ReadZFARMCount(uint32_t& nCount);

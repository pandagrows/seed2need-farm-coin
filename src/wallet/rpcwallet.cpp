...
#include "wallet.h"
#include "walletdb.h"
#include "zfarmchain.h"

#include <stdint.h>

#include "libzerocoin/Coin.h"
#include "spork.h"
#include "zfarm/deterministicmint.h"
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nDEPRECATED. Returns the current SEED2NEED address for receiving payments to this account.\n"

            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"

            "\nResult:\n"
            "\"seed2needaddress\"   (string) The account seed2need address\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            "setaccount \"seed2needaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"seed2needaddress\"  (string, required) The seed2need address to be associated with an account.\n"
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "getaccount \"seed2needaddress\"\n"
            "\nDEPRECATED. Returns the account associated with the given address.\n"

            "\nArguments:\n"
            "1. \"seed2needaddress\"  (string, required) The seed2need address for account lookup.\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"seed2needaddress\"  (string) a seed2need address associated with the given account\n"
            "  ,...\n"
            "]\n"
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            ...
            "2. \"amount\"      (numeric, required) The amount in FARM to send. eg 0.1\n"
...
UniValue delegatestake(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 6)
        throw std::runtime_error(
            ...
            "2. \"amount\"              (numeric, required) The amount in FARM to delegate for staking. eg 100\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw std::runtime_error(
            "sendtoaddressix \"seed2needaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "      amount,                 (numeric) The amount in FARM\n"
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount   (numeric) The total amount in FARM received at this address.\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in FARM received for this account.\n"
...
UniValue getcoldstakingbalance(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "amount              (numeric) The total amount in FARM received for this account in P2CS contracts.\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw std::runtime_error(
            ...
            "3. amount            (numeric, required) Quantity of FARM to move between accounts.\n"
            ...
            "\nExamples:\n"
            "\nMove 0.01 FARM from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 FARM from timotei to akiko with a comment\n" +
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toseed2needaddress\" amount ( minconf \"comment\" \"comment-to\" includeDelegated)\n"
            "\nDEPRECATED (use sendtoaddress). Send an amount from an account to a seed2need address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
            "2. \"toseed2needaddress\"  (string, required) The seed2need address to send funds to.\n"
            "3. amount                (numeric, required) The amount in FARM. (transaction fee is added on top).\n"
            "4. minconf               (numeric, optional, default=1) Only use funds with at least this many confirmations.\n"
            ...
            "\nExamples:\n"
            "\nSend 0.01 FARM from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The seed2need address is the key, the numeric amount in FARM is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw std::runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in FARM received by the address\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 6)
        throw std::runtime_error(
            ...
            "    \"address\":\"seed2needaddress\",    (string) The seed2need address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in FARM. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in FARM. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw std::runtime_error(
            ...
            "    \"amount\": x.xxx,          (numeric) The amount in FARM. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in FARM. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in FARM\n"
            ...
            "      \"amount\" : x.xxx                  (numeric) The amount in FARM\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw std::runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending FARMs\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw std::runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending FARMs\n" +
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending FARMs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in FARM/kB rounded to the nearest 0.00000001\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,                      (numeric) the total FARM balance of the wallet (cold balance excluded)\n"
            "  \"delegated_balance\": xxxxx,              (numeric) the FARM balance held in P2CS (cold staking) contracts\n"
            "  \"cold_staking_balance\": xx,              (numeric) the FARM balance held in cold staking addresses\n"
            "  \"unconfirmed_balance\": xxx,              (numeric) the total unconfirmed balance of the wallet in FARM\n"
            "  \"immature_delegated_balance\": xxxxxx,    (numeric) the delegated immature balance of the wallet in FARM\n"
            "  \"immature_cold_staking_balance\": xxxxxx, (numeric) the cold-staking immature balance of the wallet in FARM\n"
            "  \"immature_balance\": xxxxxx,              (numeric) the total immature balance of the wallet in FARM\n"
            "  \"txcount\": xxxxxxx,                      (numeric) the total number of transactions in the wallet\n"
            "  \"keypoololdest\": xxxxxx,                 (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,                     (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,                   (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,                      (numeric) the transaction fee configuration, set in FARM/kB\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw std::runtime_error(
            ...
            "multisend <SEED2NEED Address> <percent>\n"
    ...
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid FARM address");
...
UniValue getzerocoinbalance(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            "getzerocoinbalance\n"
            "\nReturn the wallet's total zFARM balance.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nResult:\n"
            "amount         (numeric) Total zFARM balance.\n"
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nList all zFARM mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
            ...
            "    \"zFARM ID\": \"xxx\",   (string) Pubcoin in hex format.\n"
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zfarmTracker->ListMints(true, fMatureOnly, true);
    ...
    if (fVerbose) {
        for (auto m : setMints) {
            ...
            objMint.push_back(Pair("zFARM ID", m.hashPubcoin.GetHex()));     // PubCoin
            ...
            if (m.hashStake == 0) {
                ...
                if (pwalletMain->GetMint(m.hashSerial, mint)) {
                    ...
                    pwalletMain->zfarmTracker->UpdateState(m);
                }
            }
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zfarmTracker->ListMints(true, true, true);
...
UniValue listspentzerocoins(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            "listspentzerocoins\n"
            "\nList all the spent zFARM mints in the wallet.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            ...
            "\nMint the specified zFARM amount\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Farm to convert to zFARM\n"
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_WALLET_ERROR, "zFARM minting is DISABLED");
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zFARM is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"value\": amount,         (numeric) Value in FARM.\n"
            "      \"address\": \"xxx\"         (string) FARM address or \"zerocoinmint\" for reminted change.\n"
            "    }\n"
            "    ,...\n"
            "  ]\n"
            "}\n"
    ...
    const bool fMintChange = params[1].get_bool();       // Mint change to zFARM
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST) {
        if (fMintChange)
            throw JSONRPCError(RPC_WALLET_ERROR, "zFARM minting is DISABLED (except for regtest), cannot mint change");

        if (!isPublicSpend)
            throw JSONRPCError(RPC_WALLET_ERROR, "zFARM old spend only available in regtest for tests purposes");
    }

    std::vector<CZerocoinMint> vMintsSelected;
    return DoZfarmSpend(nAmount, fMintChange, fMinimizeChange, vMintsSelected, address_str, isPublicSpend);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            ...
            "\nSpend zFARM mints to a FARM address.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    return DoZfarmSpend(nAmount, false, true, vMintsSelected, address_str, isPublicSpend);
}


extern UniValue DoZfarmSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzFARMTracker* zfarmTracker = pwalletMain->zfarmTracker.get();
    std::set<CMintMeta> setMints = zfarmTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzFARMTracker* zfarmTracker = pwalletMain->zfarmTracker.get();
    std::set<CMintMeta> setMints = zfarmTracker->ListMints(false, false, false);
    ...
    for (CZerocoinSpend spend : listUnconfirmedSpends) {
        for (auto& meta : setMints) {
            if (meta.hashSerial == GetSerialHash(spend.GetSerial())) {
                zfarmTracker->SetPubcoinNotUsed(meta.hashPubcoin);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"denomination\"         (integer, optional) Export a specific denomination of zFARM\n"

            "\nResult:\n"
            "[                   (array of json object)\n"
            "  {\n"
            "    \"id\": \"serial hash\",  (string) the mint's zFARM serial hash \n"
            ...
            "    \"v\": version,   (numeric) The version of the zFARM\n"
            "    \"k\": \"privkey\"  (string) The zFARM private key (V2+ zFARM only)\n"
            "  }\n"
            "  ,...\n"
            "]\n"
    ...
    CzFARMTracker* zfarmTracker = pwalletMain->zfarmTracker.get();
    std::set<CMintMeta> setMints = zfarmTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() == 0)
        throw std::runtime_error(
            ...
            "  \"value\": amount    (numeric) The total zFARM value of zerocoin mints that were added\n"
            "}\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zfarmTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw std::runtime_error(
            "reconsiderzerocoins\n"
            "\nCheck archived zFARM list to see if any mints were added to the blockchain.\n" +
            HelpRequiringPassphrase() + "\n"
...
UniValue setzfarmseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw std::runtime_error(
            "setzfarmseed \"seed\"\n"
            "\nSet the wallet's deterministic zfarm seed to a specific value.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zfarm seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzfarmseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzfarmseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzFARMWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzfarmseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw std::runtime_error(
            "getzfarmseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zFARM seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzfarmseed", "") + HelpExampleRpc("getzfarmseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw std::runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zFARM seed.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zFARM to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zFARM to generate.\n"
    ...
    CzFARMWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzfarmstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                "dzfarmstate\n"
                        "\nThe current state of the mintpool of the deterministic zFARM wallet.\n" +
                HelpRequiringPassphrase() + "\n"
    ...
    obj.push_back(Pair("dzfarm_count", nCount));
...
void static SearchThread(CzFARMWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZFARM(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzfarm(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw std::runtime_error(
            "searchdzfarm\n"
            "\nMake an extended search for deterministically generated zFARM that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassphrase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zFARM to start with.\n"
            "2. \"range\"       (numeric) How many zFARM to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzfarm", "1, 100, 2") + HelpExampleRpc("searchdzfarm", "1, 100, 2"));
    ...
    boost::thread_group* dzfarmThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzfarmThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzfarmThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zfarmTracker->GetSerialHashes());
...
UniValue createautomintaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
                ...
                "\nResult\n"
                "\"address\"     (string) SEED2NEED address for auto minting\n" +
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 7)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) SEED2NEED address to spend to. If not specified, "
    ...
    return DoZfarmSpend(mint.GetDenominationAsAmount(), false, true, vMintsSelected, address_str, isPublicSpend);
}

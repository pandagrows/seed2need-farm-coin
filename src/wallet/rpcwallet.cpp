...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturns the current SEED2NEED address for receiving payments to this account.\n"
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw std::runtime_error(
            "sendfrom \"fromaccount\" \"toseed2needaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a seed2need address.\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total FARM balance of the wallet\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zFARM
    if (fMintChange && Params().NetworkID() != CBaseChainParams::REGTEST)
        throw JSONRPCError(RPC_WALLET_ERROR, "zFARM minting is DISABLED, cannot mint change");
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZfarmSpend(nAmount, false, true, vMintsSelected, address_str);
}
...
UniValue spendrawzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 4 || params.size() > 5)
        throw std::runtime_error(
            ...
            "5. \"address\"          (string, optional) SEED2NEED address to spend to. If not specified, spend to change add.\n"
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 0)
        throw std::runtime_error(
            "clearspendcache\n"
            "\nClear the pre-computed zFARM spend cache, and database.\n" +
            HelpRequiringPassphrase() + "\n"
...

    {
        ...
        while (nTries < 100) {
            ...
            if (fLocked) {
                if (zfarmTracker->ClearSpendCache()) {

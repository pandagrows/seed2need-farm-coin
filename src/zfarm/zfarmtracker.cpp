...
CzFARMTracker::CzFARMTracker(std::string strWalletFile)
{
...
CzFARMTracker::~CzFARMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzFARMTracker::Init()
{
...
bool CzFARMTracker::Archive(CMintMeta& meta)
{
...
bool CzFARMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzFARMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzFARMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzFARMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzFARMTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzFARMTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzFARMTracker::GetSerialHashes()
{
...
CAmount CzFARMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzFARMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzFARMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzFARMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzFARMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzFARMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzFARMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzFARMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzFARMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzFARMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzFARMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzFARMWallet* zFARMWallet)
{
    bool iszFARMWalletInitialized = (NULL != zFARMWallet);
    CMintMeta meta;
    ...
    if (! iszFARMWalletInitialized)
        zFARMWallet = new CzFARMWallet(strWalletFile);
    meta.isSeedCorrect = zFARMWallet->CheckSeed(dMint);
    if (! iszFARMWalletInitialized)
        delete zFARMWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzFARMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzFARMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzFARMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzFARMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzFARMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzFARMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzFARMWallet* zFARMWallet = new CzFARMWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zFARMWallet);
        }
        delete zFARMWallet;
        LogPrint("zero", "%s: added %d dzfarm from DB\n", __func__, listDeterministicDB.size());
    }

...
CzFARMWallet::CzFARMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zfarm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZFARMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZFARMSeed_deprecated()) {
                    LogPrintf("%s: Updated zFARM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zfarm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zfarm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzFARMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZFARMCount(nCountLastUsed);
    else if (!walletdb.ReadZFARMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzFARMWallet::Lock()
{
    seedMaster = 0;
}

void CzFARMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzFARMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZFARM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzFARMWallet::LoadMintPoolFromDB()
{
...
void CzFARMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzFARMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzFARMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zfarmTracker->HasPubcoinHash(pMint.first)) {
...
bool CzFARMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZFARM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    pwalletMain->zfarmTracker->Add(dMint, true);
...
void CzFARMWallet::SeedToZFARM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzFARMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzFARMWallet::GenerateDeterministicZFARM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzFARMWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzFARMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzFARMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

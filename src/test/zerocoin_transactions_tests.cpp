...
BOOST_AUTO_TEST_CASE(zerocoin_public_spend_test)
{
    ...
    CTxIn in;
    if (!ZFARMModule::createInput(in, mint, tx.GetHash())){
        BOOST_CHECK_MESSAGE(false, "Failed to create zc input");
    }

    PublicCoinSpend publicSpend(ZCParams);
    if (!ZFARMModule::validateInput(in, out, tx, publicSpend)){
        BOOST_CHECK_MESSAGE(false, "Failed to validate zc input");
    }

    PublicCoinSpend publicSpendTest(ZCParams);
    BOOST_CHECK_MESSAGE(ZFARMModule::parseCoinSpend(in, tx, out, publicSpendTest), "Failed to parse public spend");
    ...
    BOOST_CHECK_MESSAGE(!ZFARMModule::validateInput(in, out, tx, publicSpend2), "Different denomination");

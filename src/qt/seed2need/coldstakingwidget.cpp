    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your FARMs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept FARM delegation / Delegate FARM"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select FARM outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 FARM" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}

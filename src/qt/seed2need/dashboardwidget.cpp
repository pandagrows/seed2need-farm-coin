    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of FARM staked and/or generated from Masternodes."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareFarm, "square-chart-farm");
    setCssProperty(ui->labelSquareMNRewards, "square-chart-mnrewards");
    setCssProperty(ui->labelFarm, "text-chart-farm");
    ...
    ui->labelAmountMNRewards->setText("0 FARM");
    ui->labelAmountFarm->setText("0 FARM");
    setCssProperty(ui->labelAmountFarm, "text-stake-farm-disable");
    ...
    ui->btnHowTo->setText(tr("How to get FARM"));
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isFarm = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZFARM && modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::MNReward;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair FARM, zFARM
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("FARM");
    set1 = new QBarSet("FARM MN");
    ...
    set0->append(chartData->valuesFarm);
    set1->append(chartData->valuesMNRewards);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalFarm > 0 || chartData->totalMNRewards > 0) {
        setCssProperty(ui->labelAmountFarm, "text-stake-farm");
        setCssProperty(ui->labelAmountMNRewards, "text-mnrewards-mnrfarm");
    } else {
        setCssProperty(ui->labelAmountFarm, "text-stake-farm-disable");
        setCssProperty(ui->labelAmountMNRewards, "text-stake-mnrewards-disable");
    }
    forceUpdateStyle({ui->labelAmountFarm, ui->labelAmountMNRewards});
    ui->labelAmountFarm->setText(GUIUtil::formatBalance(chartData->totalFarm, nDisplayUnit));
    ui->labelAmountMNRewards->setText(QString::number(chartData->totalMNRewards / COIN) +" FARM MNR");

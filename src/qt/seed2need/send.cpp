    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("FARM");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zFARM");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (FARM) or private coins (zFARM)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("SEED2NEED address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 FARM");
    ...
    setCssProperty(coinIcon, "coin-icon-farm");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onFARMSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onFARMSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send FARM");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zFARM");
...
void SendWidget::refreshAmounts() {
    ...
    bool isZfarm = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZfarm));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZfarm ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendFarm = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendFarm ? AskPassphraseDialog::Context::Send_FARM : AskPassphraseDialog::Context::Send_zFARM, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendFarm) ? send(recipients) : sendZfarm(recipients)) {
...
bool SendWidget::sendZfarm(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zFARM is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    if (walletModel->sendZfarm(
        ...
        inform(tr("zFARM transaction sent!"));
        ZFarmControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZFARM_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zFARM require a security level of 100 to successfully spend.");
        } else {
        ...
        emit message("zFARM transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isFARM){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any FARM to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ...
            zFarmControl->exec();
            ui->btnCoinControl->setActive(!ZFarmControlDialog::setSelectedMints.empty());
            zFarmControl->deleteLater();
        } else {
            inform(tr("You don't have any zFARM in your balance to select."));
        }
    }
}
...
void SendWidget::onFARMSelected(bool _isFARM){
    isFARM = _isFARM;
    setCssProperty(coinIcon, _isFARM ? "coin-icon-farm" : "coin-icon-zfarm");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress farmAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(farmAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(farmAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {

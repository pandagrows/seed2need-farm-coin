...
#include "qt/seed2need/privacywidget.h"
#include "qt/seed2need/forms/ui_privacywidget.h"
#include "qt/seed2need/qtutils.h"
#include "guiutil.h"
#include "qt/seed2need/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(SEED2NEEDGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zFARM anonymizes your FARM by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zFARM or convert back to FARM"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 FARM ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zFARM");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zFARM");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zFARM");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zFARM");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zFARM");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zFARM");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zFARM");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zFARM");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zFARM Movements"));
    ...
    // Only Convert to FARM enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzFARM->setTitleClassAndText("btn-title-grey", "Total 0 zFARM");
    ui->btnTotalzFARM->setSubTitleClassAndText("text-subtitle", "Show denominations of zFARM owned.");
    ui->btnTotalzFARM->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select FARM outputs to mint into zFARM.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zFARM");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzFARM, SIGNAL(clicked()), this, SLOT(onTotalZfarmClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zFARM");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of FARM to mint into zFARM"));
    }else{
        btnText = tr("Convert back to FARM");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zFARM to convert back into FARM"));
    }
...
void PrivacyWidget::onTotalZfarmClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzFARM->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzFARM->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zFARM is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zFARM, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zFARM").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zFARM minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZfarm(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zFARM converted back to FARM"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    ui->btnTotalzFARM->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}

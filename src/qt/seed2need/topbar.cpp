    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopFarm, ui->labelAmountTopzFarm}, "amount-small-topbar");
    setCssProperty({ui->labelAmountFarm, ui->labelAmountzFarm}, "amount-topbar");
    setCssProperty({ui->labelPendingFarm, ui->labelPendingzFarm, ui->labelImmatureFarm, ui->labelImmaturezFarm}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelAmountTopzFarm->setVisible(false);
    ui->labelAmountzFarm->setVisible(false);
    ui->labelPendingzFarm->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // FARM Balance
    CAmount farmAvailableBalance = balance;
    // zFARM Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalFarm = GUIUtil::formatBalance(farmAvailableBalance, nDisplayUnit);
    //QString totalzFarm = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopFarm->setText(totalFarm);
    //ui->labelAmountTopzFarm->setText(totalzFarm);

    // Expanded
    ui->labelAmountFarm->setText(totalFarm);
    //ui->labelAmountzFarm->setText(totalzFarm);

    ui->labelPendingFarm->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    //ui->labelPendingzFarm->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureFarm->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    //ui->labelImmaturezFarm->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}

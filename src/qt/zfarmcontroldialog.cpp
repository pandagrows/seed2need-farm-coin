...
#include "ui_zfarmcontroldialog.h"
...
std::set<std::string> ZFarmControlDialog::setSelectedMints;
std::set<CMintMeta> ZFarmControlDialog::setMints;

bool CZFarmControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZFarmControlDialog::COLUMN_DENOMINATION || column == ZFarmControlDialog::COLUMN_VERSION || column == ZFarmControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZFarmControlDialog::ZFarmControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZFarmControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zFARM Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZFarm->setProperty("cssClass", "text-main-purple");
    ui->labelZFarm_int->setProperty("cssClass", "text-main-purple");
...
ZFarmControlDialog::~ZFarmControlDialog()
{
    delete ui;
}

void ZFarmControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFarmControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZFarmControlWidgetItem* itemDenom(new CZFarmControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZFarmControlWidgetItem *itemMint = new CZFarmControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zFARM.";
            else if (!mint.isSeedCorrect)
                strReason = "The zFARM seed used to mint this zFARM is not the same as currently hold in the wallet";
            else
...
void ZFarmControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFarmControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZFarm_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZFarmControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZFarmControlDialog::GetSelectedMints()
{
...
void ZFarmControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 FARM.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
bool WalletModel::createZfarmSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZfarm(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzFARMReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzFARMReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzFARMReset.connect(boost::bind(NotifyzFARMReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzFARMReset.disconnect(boost::bind(NotifyzFARMReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zfarmTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}

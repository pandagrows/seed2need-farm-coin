...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZfarm){
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZfarm)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZfarm);
}
...
bool parseBitcoinURI(const QUrl& uri, SendCoinsRecipient* out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::FARM, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::FARM, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
bool openConfigfile()
{
    ...
    /* Open seed2need.conf with the associated application */

...
*Setup instructions for a gitian build of SEED2NEED using a Debian VM or physical system.*
...
- [Building SEED2NEED](#building-seed2need)
...
Clone the git repositories for seed2need and gitian and then checkout the seed2need version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/seed2need-crypto/seed2need.git
cd seed2need
...
Follow the instructions in [doc/release-process.md](release-process.md) in the seed2need repository
...
To build SEED2NEED (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the seed2need repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/seed2need/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/seed2need-crypto/seed2need
...
For example:
```bash
URL=https://github.com/crowning-/seed2need.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit seed2need=${COMMIT} --url seed2need=${URL} ../seed2need/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit seed2need=${COMMIT} --url seed2need=${URL} ../seed2need/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit seed2need=${COMMIT} --url seed2need=${URL} ../seed2need/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/seed2need-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/seed2need-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/seed2need-build.assert
```
...
[seed2need/gitian.sigs](https://github.com/seed2need-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
